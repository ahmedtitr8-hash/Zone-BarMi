...
/** يتحقق أن مفتاح ClearKey عبارة عن 32 حرف hex فقط (16 بايت AES-128) — يمنع حقن أوامر عبر هذا الحقل */
...
function buildFfmpegArgs(streamUrl, logo, outputPath, decryptionKey) {
...
function buildLiveFfmpegArgs(streamUrl, hlsDir, logo, rtmpUrl, decryptionKey) {
  const isHttp = /^https?:\/\//i.test(streamUrl);
  const args = ['-y'];
...
function startLiveProcess(club, streamUrl, hlsDir, logo, rtmpUrl, decryptionKey) {
  const args = buildLiveFfmpegArgs(streamUrl, hlsDir, logo, rtmpUrl, decryptionKey);
...
function startRecordingPipeline(id, rec, streamUrl, logo, decryptionKey) {
  const key = `${rec.club}/${rec.matchName}.mp4`;
  rec.resultKey = key;
  rec.tempFilePath = path.join(TEMP_DIR, `rec_${id}.mp4`);

  const args = buildFfmpegArgs(streamUrl, logo, rec.tempFilePath, decryptionKey);
...
function beginRecordingNow(id) {
  ...
  startRecordingPipeline(id, rec, rec.url, logo, rec.decryptionKey);
...
app.post('/api/preview/frame', requireApiKey, async (req, res) => {
  const { url, decryptionKey } = req.body;
  if (!isValidStreamUrl(url)) {
    return res.status(400).json({ error: 'رابط غير صالح' });
  }
  if (decryptionKey && !isValidClearKey(decryptionKey)) {
    return res.status(400).json({ error: 'مفتاح فك التشفير غير صالح (لازم يكون 32 حرف hex)' });
  }
  ...
  try {
    ...
    const frameArgs = [
      '-y',
      ...(decryptionKey ? ['-decryption_key', decryptionKey.trim()] : []),
      '-i', url.trim(),
      '-frames:v', '1',
...
app.post('/api/live/start', requireApiKey, logoUpload.single('logo'), requireValidClub, (req, res) => {
  const { url, logoX, logoY, logoW, logoH, videoWidth, videoHeight, decryptionKey } = req.body;
  ...
  const decKey = decryptionKey ? decryptionKey.trim() : null;
  const live = {
    url: url.trim(),
  ...
  live.process = startLiveProcess(req.clubKey, live.url, hlsDir, logo, okruUrl, decKey);
...
app.post('/api/record/start', requireApiKey, logoUpload.single('logo'), requireValidClub, (req, res) => {
  const { url, matchName, startAt, stopAt, logoX, logoY, logoW, logoH, videoWidth, videoHeight, decryptionKey } = req.body;
  ...
  const rec = {
    ...
    logoPath: req.file ? req.file.path : null,
    logoPos,
    decryptionKey: decryptionKey ? decryptionKey.trim() : null,
  };
  activeRecordings.set(id, rec);
