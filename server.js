...
async function uploadSegmentPart(id, fileName, partIndex) {
  ...
  const partKey = `_parts_tmp/${rec.club}/${id}/${fileName}`;
  ...
  try {
    ...
    await upload.done();
    rec.uploadedParts.push({ index: partIndex, key: partKey, fileName });
 ...
 *  بنفس مخطط الاسم السابق (توافق كامل مع أي شي يعتمد على الرابط)، وينضّف كل شي محلي.
 *  المقاطع الفردية تضل محفوظة على B2 كنسخة احتياطية لين ما ينجح الدمج النهائي فعليًا —
 *  لو صار خطأ بالدمج، التسجيل مو ضايع، بس محتاج دمج يدوي لاحقًا (المقاطع موجودة). */
  ...
  try {
    ...
    for (const part of rec.uploadedParts) {
      ...
      if (!fs.existsSync(localPath)) {
        await downloadObjectToFile(part.key, localPath);
      }
    ...
    const upload = new Upload({
      ...
      params: {
        Bucket: BUCKET,
        Key: rec.resultKey,
        Body: fs.createReadStream(finalLocalPath),
        ContentType: 'video/mp4',
      },
      partSize: 8 * 1024 * 1024,
      queueSize: 3,
    ...
    // خلاص الدمج النهائي نجح ورفع — نحذف نسخ المقاطع الاحتياطية من B2 (وفّر مساحة)
    for (const part of rec.uploadedParts) {
      deleteObjectSafe(part.key).catch(() => {});
    }
  } catch (err) {
    rec.status = 'error';
    ...
    notifyTelegram(
      ...
      `⚠️ المقاطع الفردية (${rec.uploadedParts.length}) لسا محفوظة بأمان على B2 بمجلد مؤقت — تواصل يدويًا لدمجها لو احتجت.`
    );
  } finally {
    fs.rm(rec.segmentDir, { recursive: true, force: true }, () => {});
