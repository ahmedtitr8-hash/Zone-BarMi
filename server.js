...
const MINIAPP_REPO = 'ahmedtitr8-hash/milan-mini-app';
...
/** يشغّل تلقائيًا Action "transcode" بمشروع الميني اب، ويمرر له رابط البث الكامل —
 *  بدون أي تدخل يدوي منك (تضغط "بدء بث" هنا بس، والباقي يصير لحاله). */
async function triggerMiniAppTranscode(fullStreamUrl, matchLabel, qualities) {
  if (!GH_PAT) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[transcode-trigger] فشل تشغيل Action الميني اب (status ${res.status}): ${text}`);
      notifyTelegram(
        `⚠️ فشل تشغيل تعدد الجودات تلقائيًا (status ${res.status}):\n${text.slice(0, 500)}\n\n` +
        `رابط الجودة الأصلية بس شغّال حاليًا:\n${fullStreamUrl}`
      );
    } else {
      console.log('[transcode-trigger] تم تشغيل Action تعدد الجودات بمشروع الميني اب بنجاح.');
    }
  } catch (err) {
