...
async function triggerMiniAppTranscode(fullStreamUrl, matchLabel) {
  if (!GH_PAT) {
  ...
  try {
    const res = await fetch(
      `https://api.github.com/repos/${MINIAPP_REPO}/actions/workflows/${MINIAPP_WORKFLOW_FILE}/dispatches`,
      {
        ...
        body: JSON.stringify({
          ref: 'main',
          inputs: { source_url: fullStreamUrl, match_label: matchLabel },
        }),
      }
    );
    if (!res.ok) {
      ...
      notifyTelegram(`⚠️ فشل تشغيل تعدد الجودات تلقائيًا — رابط الجودة الأصلية بس شغّال حاليًا:\n${fullStreamUrl}`);
    } else {
