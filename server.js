...
/** يعيد محاولة أي عملية شبكة مرة وحدة لو فشلت أول مرة (شبكة GitHub Actions أحيانًا
 *  تهتز لحظيًا مع ملفات كبيرة) — بدل ما نستسلم من أول فشلة عابرة */
/** يفحص هل سيرفر تيليجرام المحلي يرد — لو مو متجاوب، يحاول يعيد تشغيله بنفسه
 *  (docker restart) قبل ما نكمل. هذا يعالج حالة تعطّل السيرفر بمنتصف التسجيل. */
async function ensureLocalTelegramServerHealthy() {
  if (!TELEGRAM_LOCAL_API_URL) return; // مافي سيرفر محلي مُفعّل أصلاً، ما نتدخل
  try {
    await fetch(TELEGRAM_LOCAL_API_URL, { signal: AbortSignal.timeout(4000) });
    return; // أي رد (حتى خطأ HTTP) يعني السيرفر حي ويرد
  } catch (_) {
    console.error('[health] سيرفر تيليجرام المحلي ما يرد — نحاول نعيد تشغيله (docker restart)');
    try {
      require('child_process').execSync('docker restart tg-local-api', { stdio: 'ignore', timeout: 15000 });
    } catch (restartErr) {
      console.error(`[health] فشلت محاولة إعادة التشغيل: ${restartErr.message}`);
    }
    await new Promise((r) => setTimeout(r, 8000)); // نعطيه وقت يقوم فعليًا قبل أي محاولة رفع
  }
}
/** يعيد محاولة أي عملية رفع لحد أقصى كبير من المحاولات (فاصل متصاعد لغاية دقيقة)،
 *  ويحاول يصلّح صحة السيرفر المحلي بين كل محاولة وأخرى — ما يستسلم إلا بعد استنفاد
 *  كل المحاولات فعليًا، لأن انقطاع الرفع لا يعني إن التسجيل نفسه فشل. */
async function forceUpload(fn, label, maxAttempts = 20) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      await ensureLocalTelegramServerHealthy();
...
async function uploadVideoToChannel(localPath, caption) {
  ...
  // مهم: سيرفر تيليجرام المحلي يرفض الرفع بدون Content-Length صريح (بعكس
  // api.telegram.org العادي اللي يقبل بدونه) — لازم نحسبه ونرسله يدويًا
  ...
  try {
    ...
    throw new Error(`رد غير متوقع من تيليجرام (status ${res.status}): ${raw.slice(0, 300) || '(فاضي تمامًا)'}`);
  }
...
async function downloadTelegramFileToPath(fileId, localPath) {
  ...
  try {
    ...
    throw new Error(`رد غير متوقع من تيليجرام عند جلب الملف (status ${infoRes.status}): ${infoRaw.slice(0, 300) || '(فاضي تمامًا)'}`);
  }
...
async function uploadSegmentPart(id, fileName, partIndex) {
  ...
  try {
    ...
    const { messageId, fileId } = await forceUpload(
      () => uploadVideoToChannel(localPath, caption),
      `رفع مقطع ${partIndex + 1}`,
      20
    );
...
async function finalizeRecording(id) {
  ...
  try {
    ...
    for (let i = 0; i < finalPaths.length; i++) {
      ...
      const { messageId } = await forceUpload(
        () => uploadVideoToChannel(finalPaths[i], `🎬 ${label}`),
