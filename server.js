...
// ==================== ربط تلقائي بمشروع الميني اب (تعدد الجودات) ====================
const GH_PAT = process.env.GH_PAT || '';
// كان يشاور على مستودع مختلف (milan-mini-app) بقيّة من قبل ما ينتقل transcode.yml
// لهذا المستودع (Zone-BarMi) نفسه — لازم يطابق دايمًا اسم المستودع الحالي.
const MINIAPP_REPO = 'ahmedtitr8-hash/Zone-BarMi';
const MINIAPP_WORKFLOW_FILE = 'transcode.yml';

/** يقرأ رابط نفق Cloudflare الحالي (يكتبه ملف الـ workflow لحظة توفره) */
function getTunnelUrl() {
  try {
    return fs.readFileSync(path.join(__dirname, 'tunnel_url.txt'), 'utf8').trim() || null;
  } catch (_) {
    return null;
  }
}

/** يشغّل تلقائيًا Action "transcode" بنفس المستودع، ويمرر له رابط البث الكامل —
 *  بدون أي تدخل يدوي منك (تضغط "بدء بث" هنا بس، والباقي يصير لحاله).
 *  club + callbackBase يُمرَّران عشان الـ workflow يقدر يبلّغ السيرفر بـ run_id
 *  ورابط master.m3u8 لحظة ما يجهزان (شوف /api/live/multiquality/register)،
 *  عشان نقدر نوقفه لاحقًا بزر واحد بدل ما يفضل شغّال لين نهاية الـ 6 ساعات. */
async function triggerMiniAppTranscode(fullStreamUrl, matchLabel, qualities, club, callbackBase) {
  if (!GH_PAT) {
    console.error('[transcode-trigger] GH_PAT غير مضبوط — تجاهلنا التشغيل التلقائي.');
    return;
  }
  const q = qualities || {};
  try {
    const inputs = {
      source_url: fullStreamUrl,
      match_label: matchLabel,
      quality_720: q.quality720 ? 'true' : 'false',
      quality_480: q.quality480 ? 'true' : 'false',
      quality_360: q.quality360 ? 'true' : 'false',
    };
    // لو تنقل السيرفر نفسه جاهز، نمرر رابط تبليغ يقدر الـ workflow يوصله وهو شغّال
    if (callbackBase && club) {
      inputs.callback_url = `${callbackBase}/api/live/multiquality/register`;
      inputs.club = club;
    }
    const res = await fetch(
      `https://api.github.com/repos/${MINIAPP_REPO}/actions/workflows/${MINIAPP_WORKFLOW_FILE}/dispatches`,
      {
        method: 'POST',
        headers: {
    ...
    if (!res.ok) {
      ...
      console.error(`[transcode-trigger] فشل تشغيل Action تعدد الجودات (status ${res.status}): ${text}`);
      notifyTelegram(
        `⚠️ فشل تشغيل تعدد الجودات تلقائيًا (status ${res.status}):\n${text.slice(0, 500)}\n\n` +
        `رابط الجودة الأصلية بس شغّال حاليًا:\n${fullStreamUrl}`
      );
    } else {
      console.log('[transcode-trigger] تم تشغيل Action تعدد الجودات بنجاح.');
    }
  } catch (err) {
    console.error(`[transcode-trigger] خطأ أثناء استدعاء GitHub API: ${err.message}`);
    notifyTelegram(
      `⚠️ فشل تشغيل تعدد الجودات تلقائيًا (خطأ اتصال): ${err.message}\n\n` +
      `رابط الجودة الأصلية بس شغّال حاليًا:\n${fullStreamUrl}`
    );
  }
}

/** يقرأ/يحدّث docs/current-stream.json (نفس الملف اللي يبنيه live-relay.yml للرابط
 *  الثابت) عشان يضيف رابط master.m3u8 الحالي لكل نادي — بدون هذا، الـ Worker
 *  (زي https://zone-barmi.ahmedtitr8.workers.dev) ما يقدر يعرف رابط تعدد الجودات
 *  المؤقت أصلاً، فما فيه طريقة يعمل لها رابط ثابت. */
const CURRENT_STREAM_JSON_PATH = path.join(__dirname, 'docs', 'current-stream.json');

function updateMultiQualityStatus(club, masterUrl /* null = احذف الإدخال */) {
  let data;
  try {
    data = JSON.parse(fs.readFileSync(CURRENT_STREAM_JSON_PATH, 'utf8'));
  } catch (e) {
    console.error('[multiquality-status] تعذّر قراءة current-stream.json:', e.message);
    return; // بدون ملف أساسي (panelUrl) ما فيه داعي نكتب واحد جديد هنا
  }
  data.multiQuality = data.multiQuality || {};
  if (masterUrl) {
    data.multiQuality[club] = masterUrl;
  } else {
    delete data.multiQuality[club];
  }
  try {
    fs.writeFileSync(CURRENT_STREAM_JSON_PATH, JSON.stringify(data, null, 2), 'utf8');
    ...
    execSync('git add docs/current-stream.json', { cwd: __dirname });
    execSync('git commit -m "تحديث رابط تعدد الجودات [skip ci]"', { cwd: __dirname });
    execSync('git push', { cwd: __dirname });
  } catch (e) {
    // فشل الحفظ بالمستودع ما يوقف تعدد الجودات نفسه — بس الرابط الثابت
    // (عبر الـ Worker) ما يتحدث لين ينجح الحفظ بمحاولة لاحقة
    console.error('[multiquality-status] فشل تحديث الملف بالمستودع:', e.message);
  }
}

/** يلغي تشغيلة "transcode" الحالية (تعدد الجودات) عن طريق GitHub API — يُستخدم من
 *  زر "إيقاف تعدد الجودات" ومن /api/live/stop العادي (يوقف الاثنين مع بعض). */
async function cancelMiniAppRun(runId) {
  if (!GH_PAT || !runId) return false;
  try {
    const res = await fetch(
      `https://api.github.com/repos/${MINIAPP_REPO}/actions/runs/${runId}/cancel`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${GH_PAT}`,
    ...
    return res.ok || res.status === 409; // 409 = ملغى/منتهي أصلاً، نعتبرها نجاح
  } catch (err) {
    console.error(`[transcode-cancel] خطأ أثناء إلغاء الـ run: ${err.message}`);
    return false;
  }
}

// ==================== إعداد Internet Archive (2026-08-23) ====================
// الفيديو النهائي يترفع لـ Internet Archive بدل تيليجرام/B2 — رابط خام مباشر دائم،
// مجاني، بدون بطاقة، بدون حد مشاهدة.
const IA_ACCESS_KEY = process.env.IA_ACCESS_KEY || '';
const IA_SECRET_KEY = process.env.IA_SECRET_KEY || '';
// هامش أمان تحت حد Internet Archive الفعلي (~5GB لكل ملف)
const IA_MAX_UPLOAD_BYTES = 4.7 * 1024 ** 3;

/** يبني معرّف عنصر فريد (item identifier) لكل تسجيل — يُستخدم كـ"مجلد" التخزين */
function buildArchiveItemId(matchName, id) {
  const safe = matchName.replace(/[^a-zA-Z0-9]/g, '-');
  return `barmi-${safe}-${id}`;
}
/** رابط التحميل/التشغيل المباشر النهائي بعد الرفع */
function archiveDownloadLink(itemId, fileName) {
  return `https://archive.org/download/${itemId}/${fileName}`;
}
/** يرفع ملف لـ Internet Archive عبر واجهتهم المتوافقة مع S3 (PUT مباشر) */
async function uploadFileToArchive(localPath, itemId, fileName) {
  if (!IA_ACCESS_KEY || !IA_SECRET_KEY) {
    throw new Error('مفاتيح Internet Archive (IA_ACCESS_KEY / IA_SECRET_KEY) غير مضبوطة.');
  }
  const stats = fs.statSync(localPath);
  const res = await fetch(`https://s3.us.archive.org/${itemId}/${encodeURIComponent(fileName)}`, {
    method: 'PUT',
    headers: {
  ...
  if (!res.ok) {
    ...
    throw new Error(`فشل الرفع لـ Internet Archive (status ${res.status}): ${text.slice(0, 300)}`);
  }
  return archiveDownloadLink(itemId, fileName);
}
/** يعيد محاولة أي عملية رفع لحد أقصى كبير من المحاولات (فاصل متصاعد لغاية دقيقة) —
 *  ما يستسلم إلا بعد استنفاد كل المحاولات فعليًا، لأن انقطاع الرفع لا يعني إن
 *  التسجيل نفسه فشل (الملف المحلي يبقى موجود). */
async function forceUpload(fn, label, maxAttempts = 20) {
  let lastErr;
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      const delay = Math.min(5000 + attempt * 4000, 60000);
...
// ==================== فهرس التسجيلات المرفوعة على Internet Archive ====================
// B2 القديم ما يُستخدم للرفع الفعلي من زمان (الرفع الحقيقي فوق عبر uploadFileToArchive) —
// بس ما كان فيه فهرس يعرف "وش المرفوع فعليًا" على IA، فقائمة/حذف/مساحة التخزين كانت
// تقرأ من B2 الفاضي بالغلط. هذا الفهرس يُحفظ كملف بنفس المستودع (يتحدث بـgit push، نفس
// أسلوب current-stream.json) عشان يبقى موجود بين تشغيلات الأكشن المختلفة.
const ARCHIVE_INDEX_PATH = path.join(__dirname, 'data', 'archive-index.json');

function readArchiveIndex() {
  try {
    return JSON.parse(fs.readFileSync(ARCHIVE_INDEX_PATH, 'utf8'));
  } catch (e) {
    return [];
  }
}

function writeArchiveIndexAndPush(index) {
  fs.mkdirSync(path.dirname(ARCHIVE_INDEX_PATH), { recursive: true });
  fs.writeFileSync(ARCHIVE_INDEX_PATH, JSON.stringify(index, null, 2), 'utf8');
  try {
    execSync('git config user.name "github-actions[bot]"', { cwd: __dirname });
    execSync('git config user.email "github-actions[bot]@users.noreply.github.com"', { cwd: __dirname });
    execSync('git add data/archive-index.json', { cwd: __dirname });
    execSync('git commit -m "تحديث فهرس التسجيلات [skip ci]"', { cwd: __dirname });
    execSync('git push', { cwd: __dirname });
  } catch (e) {
    // فشل الحفظ بالمستودع ما يوقف التسجيل نفسه — الملف يبقى محلي بس لهذي الجلسة
    console.error('[archive-index] فشل حفظ الفهرس بالمستودع:', e.message);
  }
}

function addToArchiveIndex(entries) {
  const index = readArchiveIndex();
  index.push(...entries);
  writeArchiveIndexAndPush(index);
}

function removeFromArchiveIndex(club, name) {
  const index = readArchiveIndex().filter((e) => !(e.club === club && e.name === name));
  writeArchiveIndexAndPush(index);
}

/** يحاول حذف الملف فعليًا من Internet Archive (أفضل جهد — ما يوقف حذف الفهرس لو فشل) */
async function deleteFileFromArchive(itemId, fileName) {
  if (!IA_ACCESS_KEY || !IA_SECRET_KEY) return;
  try {
    await fetch(`https://s3.us.archive.org/${itemId}/${encodeURIComponent(fileName)}`, {
      method: 'DELETE',
      headers: { authorization: `LOW ${IA_ACCESS_KEY}:${IA_SECRET_KEY}` },
    });
  } catch (e) {
    console.error('[archive-index] فشل حذف الملف من Internet Archive:', e.message);
  }
}
...
// نمط آمن لاسم القناة/البث (يمنع أي محاولة اختراق مسار مجلدات عن طريق الاسم، زي ../)
const SAFE_SLOT_NAME = /^[a-zA-Z0-9_-]{1,40}$/;

function requireValidClub(req, res, next) {
  const club = req.body.club || req.query.club || req.params.club;
  // أي اسم آمن مسموح الآن (مو بس club1/club2) — يسمح بعدد غير محدود من البثوث المتزامنة،
  // كل وحدة بمجلد ffmpeg مستقل (صفر تأثير على أداء البثوث الثانية).
  if (!club || !SAFE_SLOT_NAME.test(club)) {
    return res.status(400).json({ error: 'اسم البث غير صالح (حروف/أرقام/شرطة بس، بدون مسافات)' });
  }
  req.clubKey = club;
  next();
}

// اسم عرض جاهز للاستخدام بأي رسالة (يرجع الاسم المسجّل بـCLUBS لو موجود، وإلا الاسم نفسه)
 ...
 *  وكل مقطع مكتمل يبقى محليًا فورًا بالتوازي مع استمرار كتابة المقطع الجاي. بعد ما
...
function startRecordingPipeline(id, rec, streamUrl, logo) {
  ...
  // نسجّله فورًا بدون ما نوقف كتابة المقطع الجاي (يبقى محليًا لين نهاية التسجيل)
  ...
  proc.on('close', (code) => {
    ...
    if (code !== 0) {
      ...
      notifyTelegram(`ℹ️ انتهى بث <b>${rec.matchName}</b> من طرف المصدر — جاري تجميع المقاطع ورفعها لـ Internet Archive`);
      finalizeRecording(id).catch(() => {});
    }
  });
...
/** يقرأ list.txt (اللي ffmpeg يضيف له سطر بكل مرة يقفل فيها مقطع)، ويسجّل أي مقطع
 *  جديد فورًا — بدون ما يوقف أو يبطّئ كتابة المقطع الجاي إطلاقًا.
 *  (يبقى محليًا؛ ما يُحذف ولا يُرفع لأي مكان إلا بنهاية التسجيل دفعة وحدة). */
...
/** يسجّل مقطع واحد مكتمل محليًا بترتيبه
...
async function uploadSegmentPart(id, fileName, partIndex) {
  ...
  // ما نرفع المقاطع لأي مكان أثناء التسجيل — تبقى محليًا فقط (على قرص المهمة)
  // لين ما ينتهي التسجيل، وقتها تُدمج وتُرفع دفعة وحدة لـ Internet Archive.
  // هذا أبسط وأخف من رفع كل مقطع لحاله، وما يحتاج اتصال شبكة مستمر أثناء التسجيل.
  rec.uploadedParts.push({ index: partIndex, fileName });
...
async function finalizeRecording(id) {
  ...
  try {
    // كل المقاطع محلية أصلاً (ما ترفع لأي مكان أثناء التسجيل) — بس نتأكد كل واحد
    // منها موجود فعليًا قبل الدمج
    const localPaths = [];
    ...
    // لو الملف النهائي أكبر من حد Internet Archive الآمن (~4.7GB)، نقسمه لأجزاء
    const finalPaths = [];
    if (stats.size > IA_MAX_UPLOAD_BYTES) {
      const numParts = Math.ceil(stats.size / IA_MAX_UPLOAD_BYTES);
    ...
    const itemId = buildArchiveItemId(rec.matchName, id);
    const links = [];
    const indexEntries = [];
    for (let i = 0; i < finalPaths.length; i++) {
      const fileName = finalPaths.length > 1 ? `part${i + 1}.mp4` : 'full.mp4';
      const label = finalPaths.length > 1 ? `الجزء ${i + 1}/${finalPaths.length}` : null;
      const partSizeBytes = fs.statSync(finalPaths[i]).size;
      const url = await forceUpload(
        () => uploadFileToArchive(finalPaths[i], itemId, fileName),
        `رفع الملف النهائي${finalPaths.length > 1 ? ` (جزء ${i + 1})` : ''}`,
        30
      );
      links.push({ label, url });
      indexEntries.push({
        club: rec.club,
        name: label ? `${rec.matchName} — ${label}` : rec.matchName,
        itemId,
        fileName,
    ...
    addToArchiveIndex(indexEntries);
    ...
    notifyTelegram(
      ...
      `⚠️ المقاطع الفردية (${rec.uploadedParts.length}) موجودة محليًا على قرص المهمة — لو المهمة لسا شغالة، تواصل بسرعة لإنقاذها يدويًا قبل ما تنتهي.`
    );
  } finally {
    fs.rm(rec.segmentDir, { recursive: true, force: true }, () => {});
...
app.get('/api/live/status', requireApiKey, (req, res) => {
  ...
  for (const [club, live] of activeLiveStreams.entries()) {
    out[club] = {
      ...
      multiQuality: live.multiQuality
        ? { masterUrl: live.multiQuality.masterUrl, startedAt: live.multiQuality.startedAt }
        : null,
    };
  }
  res.json({ live: out });
...
app.post('/api/live/start', requireApiKey, logoUpload.single('logo'), requireValidClub, (req, res) => {
  ...
  const live = {
    ...
    multiQuality: null, // يتعبّى لاحقًا من /api/live/multiquality/register لو انطلب تعدد جودات
  };
  ...
  const fullStreamUrl = tunnelUrl ? `${tunnelUrl}/live/${req.clubKey}/stream.m3u8` : null;
  notifyTelegram(
    `📡 بدأ بث مباشر لـ <b>${clubLabel(req.clubKey)}</b>${okruNote}` +
    (fullStreamUrl ? `\n\nرابط البث (جودة أصلية، للأدمن مباشرة):\n${fullStreamUrl}` :
      '\n\n⚠️ رابط النفق لسا ما جاهز — افتح رسالة "رابط البث الجاهز الآن" وضيف له /live/' + req.clubKey + '/stream.m3u8 بنفسك.')
  );
  const wantsMultiQuality =
    req.body.quality720 === 'true' ||
    req.body.quality480 === 'true' ||
    req.body.quality360 === 'true';
  // نشغّل Action تعدد الجودات بس لو فعلاً فيه جودة إضافية مطلوبة — قبل هذا الشرط كان
  // يتشغّل دايمًا (حتى بدون أي جودة مختارة)، فيطلع رابط/نفق إضافي بلا داعي ويبني
  // master.m3u8 بجودة أصلية بس، وهذا بالضبط اللي كان يسبب اللخبطة والتأخير الإضافي
  // لمين ما يحتاج تعدد جودات أصلًا.
  // بما إن اللوقو ما يُستخدم إطلاقاً، تعدد الجودات ياخذ رابط المصدر الأصلي مباشرة (live.url)
  // بدل رابط أداة التسجيل (fullStreamUrl) — يشيل قفزة تأخير كاملة (لا داعي لتمريره عبر
  // نفق هذي الأداة أصلاً لو ما فيه لوقو نضيفه عليه).
  if (wantsMultiQuality) {
    triggerMiniAppTranscode(live.url, clubLabel(req.clubKey), {
      quality720: req.body.quality720 === 'true',
      quality480: req.body.quality480 === 'true',
      quality360: req.body.quality360 === 'true',
    }, req.clubKey, tunnelUrl);
  }
  res.json({
    success: true,
...
// يستدعيها transcode.yml لحاله لحظة ما يجهز نفقه، عشان نعرف run_id ورابط master.m3u8
// ونقدر نطفيه بزر بدل ما يفضل شغّال لين نهاية الـ 6 ساعات. محمي بنفس مفتاح الـ API.
app.post('/api/live/multiquality/register', requireApiKey, (req, res) => {
  const { club, runId, masterUrl } = req.body || {};
  if (!club || !SAFE_SLOT_NAME.test(club) || !runId || !masterUrl) {
    return res.status(400).json({ error: 'club/runId/masterUrl مطلوبين' });
  }
  const live = activeLiveStreams.get(club);
  if (!live) return res.status(404).json({ error: 'مافيه بث مباشر شغال لهذا النادي' });
  live.multiQuality = { runId, masterUrl, startedAt: Date.now() };
  updateMultiQualityStatus(club, masterUrl);
  res.json({ success: true });
});

// إيقاف تعدد الجودات بس (بدون التأثير على البث الأصلي)
app.post('/api/live/multiquality/stop', requireApiKey, requireValidClub, async (req, res) => {
  const live = activeLiveStreams.get(req.clubKey);
  if (!live || !live.multiQuality) {
    return res.status(404).json({ error: 'مافيه تعدد جودات شغال لهذا النادي' });
  }
  const ok = await cancelMiniAppRun(live.multiQuality.runId);
  live.multiQuality = null;
  updateMultiQualityStatus(req.clubKey, null);
  if (!ok) return res.status(502).json({ error: 'تعذّر إلغاء الـ Action — جرب توقفه يدويًا من تبويب Actions' });
  res.json({ success: true });
});
...
app.post('/api/live/stop', requireApiKey, requireValidClub, async (req, res) => {
  ...
  stopOkruRelay(live);
  // نوقف تعدد الجودات المرتبط معه (لو موجود) بنفس الضغطة، بدل ما يفضل شغّال لحاله
  if (live.multiQuality) {
    cancelMiniAppRun(live.multiQuality.runId).catch(() => {});
    updateMultiQualityStatus(req.clubKey, null);
  }
  ...
  notifyTelegram(`⏹ تم إيقاف البث المباشر لـ <b>${clubLabel(req.clubKey)}</b>`);
  res.json({ success: true });
});
...
app.post('/api/live/recording/start', requireApiKey, requireValidClub, (req, res) => {
  ...
  notifyTelegram(`🔴 بدأ تسجيل شوط <b>${finalName}</b> (${clubLabel(req.clubKey)})`, stopKeyboard(id));
...
app.get('/api/recordings/:club', requireApiKey, requireValidClub, async (req, res) => {
  try {
    const items = readArchiveIndex()
      .filter((e) => e.club === req.clubKey)
    ...
    res.status(500).json({ error: `تعذر جلب القائمة: ${err.message}` });
  }
});

// حذف تسجيل مرفوع على Internet Archive — نحذفه من الفهرس (المصدر الحقيقي للقائمة)
// ونحاول حذفه فعليًا من IA كمان (أفضل جهد، ما يوقف حذف الفهرس لو فشل هذا الجزء)
app.delete('/api/recordings/:club/:name', requireApiKey, requireValidClub, async (req, res) => {
  const name = decodeURIComponent(req.params.name);
  try {
    const index = readArchiveIndex();
    const entry = index.find((e) => e.club === req.clubKey && e.name === name);
    if (!entry) {
      return res.status(404).json({ error: 'التسجيل غير موجود بالفهرس أصلًا' });
    }
    await deleteFileFromArchive(entry.itemId, entry.fileName);
    removeFromArchiveIndex(req.clubKey, name);
    res.json({ success: true });
  } catch (err) {
...
// استهلاك التخزين الإجمالي — Internet Archive ما يفرض حد ثابت (بعكس B2)، فهذا رقم
// إجمالي إعلامي بس (حجم كل شيء رفعناه فعليًا)، مو نسبة من حد أقصى
app.get('/api/storage/usage', requireApiKey, async (req, res) => {
  try {
    const usedBytes = readArchiveIndex().reduce((sum, e) => sum + (e.sizeBytes || 0), 0);
    res.json({ usedBytes, quotaBytes: null });
  } catch (err) {
