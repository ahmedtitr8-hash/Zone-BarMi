...
// شريط تنبيه ثابت أعلى الصفحة — يظهر بس لو فشل الاتصال الفعلي بالسيرفر (مو أخطاء منطقية
// عادية زي "رابط غير صالح")، عشان المستخدم ما يبقى يشوف صفحة "واقفة" بدون أي تفسير.
// بعد إصلاح 2026-08-14 (السيرفر ما يعود ينهار من الأخطاء)، هذا يبقى شبكة أمان احتياطية بس.
...
function setConnBanner(show, text) {
  ...
  if (!el) {
    ...
    el.id = 'connBanner';
    el.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:9999;background:#b91c1c;' +
      'color:#fff;text-align:center;padding:10px 16px;font-weight:bold;display:none;';
...
async function api(path, options = {}) {
  ...
  try {
    ...
    // fetch نفسه فشل = تعذر الوصول للسيرفر أصلاً (مو رد بخطأ، لا رد إطلاقًا)
    consecutiveNetworkFailures++;
    if (consecutiveNetworkFailures >= 2) {
      setConnBanner(true, '⚠️ تعذر الاتصال بالسيرفر — تأكد إن الأكشن لسا شغال بـ GitHub وإن رابط الصفحة صحيح');
    }
...
function buildClubPanel(key, name) {
  ...
  panel.innerHTML = `
    ...
    <div class="card">
      ...
      <label>تعدد الجودات بالميني اب (اختر أي جودات إضافية تبيها)</label>
      <div class="quality-checks">
        <label><input type="checkbox" class="p-quality-720"> 720p</label>
        <label><input type="checkbox" class="p-quality-480" checked> 480p</label>
        <label><input type="checkbox" class="p-quality-360"> 360p</label>
      </div>
...
async function startLive(clubKey, container) {
  ...
  const quality720 = container.querySelector('.p-quality-720').checked;
  const quality480 = container.querySelector('.p-quality-480').checked;
  const quality360 = container.querySelector('.p-quality-360').checked;
  ...
  try {
    ...
    form.append('quality720', quality720 ? 'true' : 'false');
    form.append('quality480', quality480 ? 'true' : 'false');
    form.append('quality360', quality360 ? 'true' : 'false');
    if (st && st.logoFile && st.confirmed) {
      form.append('logo', st.logoFile);
      form.append('logoX', st.xPct);
      form.append('logoY', st.yPct);
